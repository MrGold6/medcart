        ...
        <content class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
            <div class="col-xl-7 col-lg-10 col-md-10 col-sm-10 mx-auto p-3 pt-3">
                <div class="card card_form shadow">
                    <div class="card-body ">
                        <c:if test="${bodyPart.id==null}">
                            <title>Add</title>
                        </c:if>
                        <c:if test="${bodyPart.id!=null}">
                            <title>Edit</title>
                        </c:if>
                        <legend class="card-title text-center">Частина тіла</legend>


                        <form:form action="/admin/add_bodyPart" method="POST" name="bodyPart"  class="was-validated">
                            <c:if test="${bodyPart.id!=null}">
                            <div class="row mb-3">
                                <label class="col-sm-6 col-form-label ln">Ід:</label>
                                <div class="col-sm-6">
                                    <input class="form-control" type="number" name="id" value="${bodyPart.id}" required>
                                </div>
                            </div>
                            </c:if>
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-6">
                                    <input type="text" name="name" class="form-control" value="${bodyPart.name}" required>
                                </div>
                            </div>

                            <center>
                                <c:if test="${bodyPart.id==null}">
                                    <input type="submit" id="in"  class="btn btn_add" name="add_bodyPart" value="Створити">
                                </c:if>
                                <c:if test="${bodyPart.id!=null}">
